package edu.utdallas.repositoryupdater;
...
public class Updates {
	...
	String[] gameComponents = { "Characters", "Lesson", "Challenge", "Locale",
			"Subject", "Theme" };
	String[] gameComponentsClose = { "</Characters>", "</Lesson>",
			"</Challenge>", "</Locale>", "</Subject>", "</Theme>" };
	String[] repoGameComponents = { "characters", "lesson", "challenge",
			"locale", "subject", "theme" };
	String[] elementsOpen = { "<character>", "<Lessons>", "<Challenges>",
			"<Locales>", "<Subjects>", "<Themes>" };
	String[] elementsClose = { "</character>", "</Lessons>", "</Challenges>",
			"</Locales>", "</Subjects>", "</Themes>" };
	String[] Games = { "MathGame", "OfficeGame", "SpaceGame", "VocabGame",
			"HistoryGame" };
	...
	public void remakeRepo() {
		...
		for (int x = 0; x < gameComponents.length; x++) {
			...
			try {
				...
				textlocation = "New Games//" + Games[0] + "//"
						+ gameComponents[x] + "MetaData.xml";
				File copyGame = new File(textlocation);
				this.checkFile(copyGame);

				File original = new File("XMLmetadata//"
						+ repoGameComponents[x] + ".xml");
	...
	private void addToXMLRepo(String newGamename, int x) {
		try {
			...
			String textlocation;
			textlocation = "New Games//" + newGamename + "//"
					+ gameComponents[x] + ".xml";
			File fileAddition = new File(textlocation);
			this.checkFile(fileAddition);
			Scanner addFile = new Scanner(fileAddition);

			String newfilelocation = "XMLrepo//" + gameComponents[x] + gameNumb
					+ ".xml";
			...
			System.out.println(gameComponents[x] + gameNumb
					+ " XMLrepo Added.\n");
			addFile.close();
			writeNewFile.close();
		}// end try
	...
	private void addtoXMLMetadata(String newGamename, int x) {
		...
		try {
			...
			File original = new File("XMLmetadata//" + repoGameComponents[x]
					+ ".xml");
			...
			String updatedfileloc = "XMLmetadata//" + repoGameComponents[x]
					+ ".xml";
	...
	private void checkFile(File validate) {
		...
		if (validate.exists() == false) {
			System.out.println("Could not find " + validate.getPath()
	...
	private void checkCreateFile(File check) {
		if (check.exists()) {
			System.out
					.println("You are trying to create a file that already exists: "
							+ check.getPath()
							+ "\n ...Incomplete Update Terminated...");
			System.exit(0);
		}
