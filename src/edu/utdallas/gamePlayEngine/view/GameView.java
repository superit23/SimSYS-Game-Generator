                ...
                if (GameAdapter.class.isInstance(arg0)) {
                    ...
                    if (msg == Message.Start) {
                        ...
                        if (((SceneAdapter) (arg0)).getScene().getMusic() != null) {
                            ...
                            String music = ((SceneAdapter) (arg0)).getScene().getMusic().getMusic();
                            AudioPlayer.playAudio(musicFolderPath + music);

                        }

                        String backDrop = ((SceneAdapter) (arg0)).getScene().getBackground().getBackground();
                    ...
                    if (msg == Message.Start) {
                        ...
                        System.out.println("View: Message ChallengeStartComplete sent");
                        gameState.setMessage(Message.StartComplete);
                        GameController.viewListener(arg0, gameState);

                    } else if (msg == Message.Play) {

                        System.out.println("View: Message ChallengePlayComplete sent");
                        gameState.setMessage(Message.PlayComplete);

                        try {
                            SandBox SBchallenge = (SandBox) gameState.getChallenegeAdapter().getChallenge();

                            if (SBchallenge.getItem().get(0).getType().equals("SandBox")) {
                                // System.out.println("QQQQQ: " + SBchallenge.getItem().get(0).getGrade());

                                int[] points = new int[18];
                                String[] xmlPoints = SBchallenge.getItem().get(0).getPoints().split(",");
                                String[] xmlAngles = SBchallenge.getItem().get(0).getAngles().split(",");

                                desiredAngle = SBchallenge.getItem().get(0).getDesiredAngle();
                                ...
                                drawTriangle = true;
                                ChallengeAdapter.quizReward = SBchallenge.getItem().get(0).getReward();

                                gameState.setQuizReward(SBchallenge.getItem().get(0).getReward());
                                gameState.setInCorrectFeedback(SBchallenge.getItem().get(0).getIncorrectFeedback());
                                gameState.setCorrectFeedBack(SBchallenge.getItem().get(0).getFeedback());
                                gameState.setPromotedTitle(SBchallenge.getItem().get(0).getPromotionTitle());
                        ...
                        QuizChallenge challenge = (QuizChallenge) gameState.getChallenegeAdapter().getChallenge();
                        ...
                        MultipleChoiceItem multiChoice = (challenge.getItem().get(0));
                        ...
                        gameViewFrame.addinformationBox(questionProp, gameState);
                            ...
                            for (GameElementAdapter adapter : gameState.getScreenAdapter().getGameElementAdapters()) {
                                ...
                                if (element.getName().contains("Character")) {
                                    ...
                                    gameViewFrame.addImage(charProp.getNameTag(),
                                            charProp.getLocation().getX(), charProp.getLocation().getY(),
                                            element.getSize().getWidth(), element.getSize().getHeight(), gameState);
                        ...
                        for (Option option : options) {
                            ...
                            System.out.println("bob:" + option.getText().getText());
                            ...
                            ChallengeAdapter.nextDisplay = option.getReward().getNextQuestion();
                            ...
                            if (option.getAssessment().toString().charAt(0) == 'C') {
                                ...
                                gameState.setCorrectFeedBack(option.getFeedback());
                                gameState.setQuizReward(option.getReward().getPoints());

                                correctValue = option.getReward().getPoints();
                                gameState.setPromotedTitle(option.getReward().getPromotionDemotion());
                                ChallengeAdapter.nextDisplay = option.getReward().getNextQuestion();

                                if (option.getReward().getNextQuestion() > 0)
                                    gameState.setNextQuestion(option.getReward().getNextQuestion());

                            } else {

                                gameState.setInCorrectFeedback(option.getFeedback());

                                ChallengeAdapter.quizFeedback = option.getFeedback();
                                // this will be displayed when no option is selected
                            }

                            gameViewFrame.addButton(optionProp, gameState, option.getAssessment().toString().charAt(0), yvalue, listCount);
                            yvalue += 140;
                            // yvalue += 5;
                        }
                        ...
                        // check if timer needed TODO

                        System.out.println("View: Message ChallengeEndComplete sent");
                        gameState.setMessage(Message.EndComplete);
                        GameController.viewListener(arg0, gameState);
                    ...
                    if (msg == Message.Start) {
                        ...
                        System.out.println("View: Message GameElementStartComplete sent");
                        gameState.setMessage(Message.StartComplete);
                        GameController.viewListener(arg0, gameState);

                    } else if (msg == Message.Play) {

                        System.out.println("View: Message GameElementPlayComplete sent");
                        gameState.setMessage(Message.PlayComplete);

                        if (!((gameState.getGameElementAdapter().getGameElement()) instanceof Prop)) {

                            gameState.setMessage(Message.PlayComplete);
                            GameController.viewListener(arg0, gameState);

                        }
                        Prop currentProp = (Prop) (gameState.getGameElementAdapter().getGameElement());
                        ...
                        if (currentProp.getName() != null) {
                            ...
                            if (currentProp.getName().startsWith("Information")) {
                                System.out.println("Calling method to add information box!!" + currentProp);

                                if (currentProp.getBehaviors() != null) {
                                    for (Behavior behavior : currentProp.getBehaviors()) {

                                        if (behavior.getBehaviorType().equals(BehaviorType.CURRENT_CHALLENGE)) {

                                            currentProp.getText().setText(ChallengeAdapter.quizFeedback);
                                        }
                                    }
                                }
                            ...
                            // Zac ZHANG: Added for ConversationBubble // TODO----

                            if (currentProp.getName().startsWith("ConversationBubble")) {
                                System.out.println("Calling method to add Conversation Bubble! " + currentProp);
                                gameViewFrame.addConversationBubble(currentProp, gameState);
                            }

                            if ((currentProp.getName()).endsWith(png)) {
                                Prop prop = (Prop) gameState.getGameElementAdapter().getGameElement();
                                ...
                                for (int i = 0; i < characterGroup.length; i++) {
                                    ...
                                    gameViewFrame.addImage(characterGroup[i], i * 150 - 20, 130, gameState);
...
//								}
                        ...
                        try {
                            try {
                                ...
                                if (currentProp.getAnimationEffect() != null && currentProp.getAnimationEffect().getSpeed() != null) {
                                    timerValue = GameAdapter.speedMap.get(currentProp.getAnimationEffect().getSpeed());
                                ...
                                if (currentProp.getName() != null
                                        && currentProp.getName().startsWith("Information")
                                        && !(localBehavior != null && localBehavior.getBehaviorType().equals(BehaviorType.TRANSITION_BEHAVIOR)))

                                    timerValue = 0;

                                // If it's a button don't proceed until click, so no need to go to EndComplete
                                if (!(propName != null && propName.trim().startsWith(Button))) {
                                    Timer timer = new Timer(timerValue,
                                            new ActionListener() {
                                                // @Override
                                                @Override
                                                public void actionPerformed(ActionEvent arg1) {
                                                    if (GameAdapter.hasGameEnded())
                                                        return;

                                                    if (localBehavior != null && localBehavior.getBehaviorType().equals(BehaviorType.TRANSITION_BEHAVIOR))
                                                        // to retain info box when button is displayed
                                                        GameViewFrame.resetLayeredPane();
                                ...
                                e.printStackTrace();
                                System.out.println("GameView Class: Exception Point 1: " + e.getMessage());
                            ...
                            e.printStackTrace();
                            System.out.println("GameView Class: Exception Point 2: " + e.getMessage());
