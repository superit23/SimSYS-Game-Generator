...
/**
 * User: clocke Date: 3/3/13 Time: 8:50 PM
 */
@XmlRootElement(name = "AssetBase")
@XmlSeeAlso({ ImageAsset.class, ButtonAsset.class, InformationBoxAsset.class,
		CharacterAsset.class, ConversationBubbleAsset.class,
		ThoughtBubbleAsset.class })
public class Asset implements Cloneable {
	...
	public Asset(GameObject gameObject) {
		...
		type = "ImageAsset";
		/*
		 * x = gameObject.getX(); y = gameObject.getY(); width =
		 * gameObject.getWidth(); height = gameObject.getHeight();
		 */
		x2 = x + width;
		y2 = y + height;
	...
	public Asset(SharedCharacter character, GameCharacter gameCharacter,
		...
		type = "CharacterAsset";
		/*
		 * x = character.getX(); y = character.getY(); width =
		 * character.getWidth(); height = character.getHeight();
		 */
		x2 = x + width;
		y2 = y + height;
		displayImage = gameCharacter.getCharacterAsset(character
				.getCharacterAssetType());
		behaviors = new ArrayList<Behavior>();
		for (ObjectMovement movement : character.getMovements()) {
			/*
			 * if(learningActCharacter.getMovementType() ==
			 * movement.getMovementType()) { //TODO needs to get adjusted for
			 * animations //behaviors.add(new Behavior(movement)); }
			 */
		}
	}
	...
	public Asset(SharedCharacter character, GameCharacter gameCharacter) {
		...
		if (character.getMovements() != null) {
			for (ObjectMovement movement : character.getMovements()) {
				// TODO Needs to get adjusted for walking animations.
	...
	public Asset(SharedInfoBox informationBox, GameText gameText) {
		this(informationBox);
		this.name = gameText.getText().getText();
	}

	public Asset(SharedButton sharedButton, GameButton gameButton) {
		this(sharedButton);
		this.name = gameButton.getText().getText();
		behaviors = new ArrayList<Behavior>();
		if (gameButton.getReward() != null) {
			Behavior behavior = new Behavior();
			behavior.setBehaviorType(BehaviorType.REWARD_BEHAVIOR);
			behavior.setAction(Integer.toString(gameButton.getReward()
					.getPoints()));

			behaviors.add(behavior);
		}
		if (gameButton.getBehavior() != null
				&& gameButton.getBehavior().getBehaviorType().value()
						.contains("Transition")) {
			Behavior behavior = new Behavior();
