...
public class SearchInput {
	...
	public void receiveTxtInput(String type) {
		...
		try {
			...
			// FileReader(type+".txt"));
			Scanner input = new Scanner(new FileReader("CurrentSearchInput\\"
					+ type + "input.txt"));
			...
			int choiceCounter = 0;
			/**
			 * do not uncomment the print statements without commenting out the
			 * input.next() statements that immediately follow them. (one needs
			 * to be commented out at all times.)
			 */
			...
			while (input.hasNextLine()) {
				...
				for (int i = 0; i < numberOfCriteria; i++) {
					// System.out.println("#"+input.next()); //list criteria
					// name
