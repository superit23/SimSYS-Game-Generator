...
import edu.utdallas.gamegeneratorcollection.ComponentCreation.FailureScreen;
...
import edu.utdallas.gamegeneratorcollection.ComponentCreation.LessonScreen;
...
import edu.utdallas.gamegeneratorcollection.ComponentCreation.ThemeStoryScreenIntro;
import edu.utdallas.gamegeneratorcollection.ComponentCreation.ThemeStoryScreenOutro;
...
/**
 * User: clocke. Date: 2/28/13 Time: 7:56 PM
 */
public class TestObjects {
	...
	private void createLocale() {
		...
		buttonButtons.put(ButtonLocationType.NEXT, new SharedButton("", 10, 10,
				10, 10, null));
		buttonButtons.put(ButtonLocationType.CHALLENGE_1, new SharedButton("",
				10, 10, 10, 10, null));
		buttonButtons.put(ButtonLocationType.CHALLENGE_2, new SharedButton("",
				10, 10, 10, 10, null));
		...
		movements.add(new ObjectMovement(ObjectMovementType.WALK_ONTO_SCREEN,
				1, 1, 5, 5));
		localeCharacters.put(LearningActCharacterType.HERO,
				new SharedCharacter(200, 200, 100, 100, null,
						LearningActCharacterType.HERO, movements));
		movements = new ArrayList<ObjectMovement>();
		movements.add(new ObjectMovement(ObjectMovementType.WALK_ONTO_SCREEN,
				1, 1, 5, 5));
		localeCharacters.put(LearningActCharacterType.VILLIAN,
				new SharedCharacter(200, 200, 100, 100, null,
						LearningActCharacterType.VILLIAN, movements));
		movements = new ArrayList<ObjectMovement>();
		movements.add(new ObjectMovement(ObjectMovementType.WALK_ONTO_SCREEN,
				1, 1, 5, 5));
		localeCharacters.put(LearningActCharacterType.ALT, new SharedCharacter(
				200, 200, 100, 100, null, LearningActCharacterType.ALT,
				movements));
		...
		SharedInfoBox informationBox = new SharedInfoBox(100, 100, 100, 100,
				"", "hero");
	...
	private void createLearningObjective() {
		...
		// screen1.getButtons().get(0).setBehavior(BehaviorType.NEXT_CHALLENGE);
		baseScreens1.add(screen);
		List<ChallengeOption> challengeOptions = new ArrayList<ChallengeOption>();
		challengeOptions.add(new ChallengeOption(ChallengeOptionType.BUTTON,
				"additional screens", new Reward(), BehaviorType.ADDITIONAL,
				baseScreens1, ButtonLocationType.CHALLENGE_1));
		challengeOptions.add(new ChallengeOption(ChallengeOptionType.BUTTON,
				"end of story", null, BehaviorType.END_OF_STORY, null,
				ButtonLocationType.CHALLENGE_2));
		/*
	...
	private void createTheme() {
		...
		buttons.put(ButtonLocationType.NEXT, new SharedButton("button intro 1",
				10, 30, 350, 350,
				new Behavior(BehaviorType.TRANSITION_BEHAVIOR)));
		...
		movements.add(new ObjectMovement(ObjectMovementType.WALK_ONTO_SCREEN,
				1, 1, 5, 5));
		themeCharacters.put(LearningActCharacterType.HERO, new SharedCharacter(
				200, 200, 100, 100, null, LearningActCharacterType.HERO,
				movements));
		movements = new ArrayList<ObjectMovement>();
		movements.add(new ObjectMovement(ObjectMovementType.WALK_OFF_SCREEN, 1,
				1, 5, 5));
		themeCharacters.put(LearningActCharacterType.VILLIAN,
				new SharedCharacter(200, 200, 100, 100, null,
						LearningActCharacterType.HERO, movements));
		movements = new ArrayList<ObjectMovement>();
		movements.add(new ObjectMovement(ObjectMovementType.WALK_ONTO_SCREEN,
				1, 1, 5, 5));
		themeCharacters.put(LearningActCharacterType.ALT, new SharedCharacter(
				200, 200, 100, 100, null, LearningActCharacterType.HERO,
				movements));
		...
		// gameButtons.get(0).setBehavior(BehaviorType.NEXT_ACT);
	...
	private List<GameButton> getGameButtons() {
		...
		// gameButton.setBehavior(BehaviorType.NEXT_SCREEN);
